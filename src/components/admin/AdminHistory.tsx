          ...
          {mockAdminHistory.map((row, i) => (
            <tr key={i} className="border-t hover:bg-gray-100 transition">
              ...
              <td className="px-4 py-2">
                <span className={`px-2 py-1 rounded text-xs font-semibold ${row.action === 'Ajout' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : row.action === 'Modification' ? 'bg-yellow-50 text-yellow-700 border border-yellow-200' : 'bg-red-50 text-red-700 border border-red-200'}`}>{row.action}</span>
              </td>
              <td className="px-4 py-2 text-gray-800">{row.user}</td>
              <td className="px-4 py-2 text-gray-800">{row.responsible}</td>
              <td className="px-4 py-2 text-gray-700">{row.details || <span className="text-neutral-400">—</span>}</td>
            </tr>
          ))}
        </tbody>
      </table>
